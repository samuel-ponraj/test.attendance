...
const SYSTEM_FIELDS = [
  { id: 'sys-name', name: 'First Name', type: 'text', required: true },
  { id: 'sys-name', name: 'Last Name', type: 'text', required: true },
...
const SystemFieldRow = ({ field }) => (
  <div className="flex items-start justify-between rounded-md border border-dashed bg-muted/20 p-3 mb-2 opacity-80">
    ...
    <div className="text-[10px] text-muted-foreground italic self-center px-2">
      Fixed
    </div>
  </div>
)
...
const SortableField = ({ field, onDelete }) => {
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className="flex items-start justify-between rounded-md border bg-card p-3 shadow-sm mb-2 group touch-none"
    >
      <div className="flex gap-3">
        <div
          {...attributes}
          {...listeners}
          className="cursor-grab active:cursor-grabbing p-1 -ml-1 mt-0.5 hover:bg-muted rounded transition-colors"
        >
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            {field.required && (
              <span className="text-[10px] text-destructive font-bold underline">
                REQ
              </span>
            )}
          </div>
          ...
          {field.options && (
            <div className="flex flex-wrap gap-1 mt-1">
              {field.options.map((opt, i) => (
                <span
                  key={i}
                  className="text-[9px] bg-muted px-1.5 py-0.5 rounded border"
                >
                  {opt}
                </span>
              ))}
...
const CreateForm = () => {
  ...
  const teamsUsedParam = searchParams.get("teamsUsed");
  const parsedTeamsUsed = teamsUsedParam
    ? JSON.parse(teamsUsedParam)
    : [];
  ...
  try {
    const finalFormTitle = editingFormId
      ? currentFormTitle
      : formTitle;
    ...
    if (editingFormId) {
      const formRef = doc(db, "customForms", editingFormId);

      const oldFormSnap = await getDoc(formRef);
      const oldFormData = oldFormSnap.data();

      const oldTeamsUsed = oldFormData?.teamsUsed || [];
      ...
      const oldTeamIds = oldTeamsUsed.map((team) => team.teamId);
      const newTeamIds = teamsUsed.map((team) => team.teamId);
      ...
      for (const teamId of removedTeamIds) {
        ...
        await updateDoc(teamRef, {
          customForms: arrayRemove(editingFormId),
          updatedAt: serverTimestamp(),
        });
      }
      ...
      for (const teamId of addedTeamIds) {
        ...
        await updateDoc(teamRef, {
          customForms: arrayUnion(editingFormId),
          updatedAt: serverTimestamp(),
        });
      }
      ...
      const formData = {
        ...
        teamsUsed,
        customFields: fields,
        adminUserId: teams.find((team) => team.id === teamsUsed[0]?.teamId)?.admin?.userId || "",
      ...
      for (const team of teamsUsed) {
        ...
        await updateDoc(teamRef, {
          customForms: arrayUnion(formRef.id),
          updatedAt: serverTimestamp(),
        });
      }
      ...
      <div className="space-y-6">
          ...
          <div className="bg-muted/30 p-4 border-b">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mt-4">
              ...
              <div className="flex flex-col sm:flex-row gap-2">
                ...
                <Button
                  type="button"
                  className="bg-success/80 hover:bg-success cursor-pointer"
                  onClick={handleSubmitForm}
                  disabled={!editingFormId && !formTitle?.trim()}
                >
        ...
        <div className="rounded-lg border bg-background overflow-hidden min-h-[400px]">
          <div className="p-3 pt-5 border-b bg-muted/10">
            <Label className="text-[12px] uppercase font-bold tracking-wider">
              Form Preview
            </Label>
          </div>
          ...
          <div className="p-3">
            {SYSTEM_FIELDS.map((sf) => (
              <SystemFieldRow key={sf.id} field={sf} />
            ))}
