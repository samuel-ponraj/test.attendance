...
const Daily = ({ teamId, team, members, initialMemberId, showBackButton = true }) => {
	...
	const getAttendanceText = (attendance) => {
		if (attendance === "present") {
			return <span className="font-semibold text-emerald-600">Present</span>;
		}

		if (attendance === "halfday") {
			return <span className="font-semibold text-orange-500">Half Day</span>;
		}
		...
		return <span className="text-muted-foreground">Not Marked</span>;
	};

	const getDailyPeriods = async () => {
		...
		while (current <= today) {
			...
			if (!isHoliday) {
				...
				if (punchSnap.exists()) {
					attendance = punchSnap.data()?.status || "not_marked";
				}
			}
	...
	const loadPeriods = async () => {
		...
		try {
			const generated = await getDailyPeriods();
	...
	const filteredPeriods = billingPeriods.filter((period) => {
		...
		const matchesAttendance =
			filterAttendance === "all"
				? true
				: period.attendance === filterAttendance;
	...
	const payablePeriods = filteredPeriods.filter(
		(period) => !period.isHoliday && period.status !== "leave"
	);
								...
								{filteredPeriods.length === 0 ? (
									...
									filteredPeriods.map((period) => {
										...
										return (
											<TableRow
												key={period.id}
												className={notApplicable ? "bg-muted/30" : ""}
											>
												...
												<TableCell className="text-center border-r">
													{holiday ? (
														...
														getAttendanceText(period.attendance)
													)}
												</TableCell>
