...
const Salary = ({ teamId, team, members }) => {
	...
	const getStatusBadge = (status) => {
		if (status === "settled") return <Badge className="bg-emerald-600">Settled</Badge>;
	...
	const fetchAttendanceSummary = async (memberId, fromDate, toDate) => {
		...
		return {
			...
			totalWorkingDays: getWorkingDaysCount(attendanceDateKeys),
			presentDays,
			absentDays,
			halfDays,
	...
	const calculateNetSalaryFromAttendance = (salaryConfig, attendance) => {
		...
		if (salaryType === "monthly") {
			...
			return {
				basicPay: monthlySalary - lossOfPay,
				perDaySalary,
				lossOfPay,
			};
		}
						...
						<CardContent>
							<div className="grid grid-cols-2 gap-4 sm:grid-cols-2 lg:grid-cols-4">
								<div className="rounded-lg border bg-muted/30 p-3">
									...
									<p className="font-semibold">{formatDate(salaryConfig.joiningDate)}</p>
								</div>

								<div className="rounded-lg border bg-muted/30 p-3">
									<p className="text-xs text-muted-foreground">Salary Type</p>
									<p className="font-semibold capitalize">{salaryConfig.salaryType || "—"}</p>
								</div>
								...
								<div className="rounded-lg border bg-muted/30 p-3">
									...
									<p className="font-semibold">{formatCurrency(salaryConfig.pf)}</p>
								</div>

								<div className="rounded-lg border bg-muted/30 p-3">
									<p className="text-xs text-muted-foreground">ESI</p>
									<p className="font-semibold">{formatCurrency(salaryConfig.esi)}</p>
								</div>

								<div className="rounded-lg border bg-muted/30 p-3">
									<p className="text-xs text-muted-foreground">Special Allowance</p>
									<p className="font-semibold">{formatCurrency(salaryConfig.specialAllowance)}</p>
								</div>

								<div className="rounded-lg border bg-muted/30 p-3">
									<p className="text-xs text-muted-foreground">Bonus</p>
									<p className="font-semibold">{formatCurrency(salaryConfig.bonus)}</p>
								</div>
							</div>
						</CardContent>
					</Card>
						...
						<Table>
							<TableHeader className="bg-muted/50">
								<TableRow>
									...
									<TableHead className="text-center border-r">Total Days in Month</TableHead>
									<TableHead className="text-center border-r">Working Days</TableHead>
										...
										<TableRow key={key}>
											...
											<TableCell className="text-center">
												{dailyAbsent ? (
													...
													<Button size="sm" variant="outline" onClick={() => openActionModal(period)}>
														Action
													</Button>
												)}
											</TableCell>
					...
					<div className="space-y-4">
						...
						{selectedAction === "pay" && (
							<div className="rounded-lg border p-4 space-y-3">
								<div className="flex flex-wrap gap-2">
									<Button variant="outline" onClick={generatePayslip} disabled={loading}>
						...
						{selectedAction === "hold" && (
							<div className="rounded-lg border p-4">
								<Button variant="outline" onClick={markAsHold} disabled={loading}>
									Mark as Hold
								</Button>
							</div>
						)}
