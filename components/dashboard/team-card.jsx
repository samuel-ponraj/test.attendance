// app/components/dashboard/team-card.jsx
"use client";
  ...
  return (
    <div className="grid grid-cols-[repeat(auto-fit,minmax(280px,1fr))] gap-4 px-4 lg:px-6">
      {sortedTeams.map((team) => {
        ...
        return (
          <Card key={team.id} className="group bg-card shadow-xs hover:shadow-md transition-all duration-300">
            ...
            <div className="px-6 pb-4 flex gap-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-muted-foreground">{team.present} Present</span>
              </div>

              <div className="flex items-center gap-2">
                <span className="w-2.5 h-2.5 rounded-full bg-destructive" />
                <span className="text-sm text-muted-foreground">{team.absent} Absent</span>
              </div>
            </div>

            <CardFooter className="flex items-center justify-between border-t">
              <span className="text-sm text-muted-foreground">{team.total} members</span>
